...
func appMain(c *cobra.Command, args []string) {
	//
	// Load Config
	//
	appCfg := app.Config{}
	...
	tftpServer := &util.TftpServer{
		...
		ReadHandler: &app.TftpHandler{
			Root: util.MustSub(tftpboot, "tftpboot"),
		},
	}
	...
	if err := ipxeRendererHandler.ParseTemplate(ipxeTemplate); err != nil {
	...
	mux.Handle("GET /tftpboot/*", http.FileServerFS(tftpboot))
